...
async function getLinkAnalytics(
  ...
  const response = await fetch(
    `/api/analytics/${linkKey}?startDate=${startDate}&endDate=${endDate}`
  );
  if (!response.ok) {
...
export function LinkAnalyticsClient({ linkKey }: Props) {
  ...
  const { data, isLoading, isError, refetch } = useQuery<LinkAnalyticsData>({
    queryKey: ["linkAnalytics", linkKey, startDate, endDate],
    queryFn: () => getLinkAnalytics(linkKey, startDate, endDate),
